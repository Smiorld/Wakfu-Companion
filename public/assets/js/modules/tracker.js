...
function hydrateTrackedItem(item) {
  ...
  if (catalogItem) {
    ...
    item.imgName = catalogItem.imgName || null;
...
function processItemLog(line) {
  ...
  trackedItems.forEach((item) => {
    ...
    if (parsed.variants.some((variant) => trackedAliases.has(variant))) {
      ...
      const resourceIconName = (item.imgName || item.name).replace(/\s+/g, "_");
      const iconPath =
        item.profession === "ALL" && item.imgId
          ? `./assets/img/items/${item.imgId}.png`
          : `./assets/img/resources/${resourceIconName}.png`;
...
function addTrackedItem() {
  ...
  trackedItems.push({
    ...
    imgName: foundItem.imgName || null,
  });
